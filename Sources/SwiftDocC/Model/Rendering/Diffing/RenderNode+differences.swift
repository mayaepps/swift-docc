    ...
    public func difference(from other: RenderNode, at path: Path) -> Differences {
        ...
        if kind != other.kind {
            let patch = JSONPatchOperation(
                variantPatchOperation: .replace(value: kind),
                pointer: JSONPointer(from: path + [CodingKeys.kind])
            )
            diffs.append(patch)
        }
        
        diffs.append(contentsOf: abstract.difference(from: other.abstract, at: path + [CodingKeys.abstract]))
//        diffs.append(schemaVersion.difference(from:other.schemaVersion, at: path + [CodingKeys.schemaVersion]))
    ...
    public func difference(from other: Array<Element>, at path: Path) -> Differences {
        ...
        let patchOperations = differences.map { diff -> JSONPatchOperation in
            ...
            case .insert(let offset, let element, _):
                let pointer = JSONPointer(from: path + [IntegerKey(offset)])
                return .add(pointer: pointer, value: AnyCodable(element as! Encodable))
            }
        }
...
private func optionalPropertyDifference<T>(_ current: T?, from other: T?, at path: Path) -> JSONPatchOperation? where T: Equatable, T: Codable {
    var difference: JSONPatchOperation? = nil
    if let current = current, let other = other {
        if current != other {
            difference = .replace(pointer: JSONPointer(from: path), value: AnyCodable(current))
        }
    } else if other != nil {
        difference = .remove(pointer: JSONPointer(from: path))
    } else if let current = current {
        difference = .add(pointer: JSONPointer(from: path), value: AnyCodable(current))
    }
    return difference
}
